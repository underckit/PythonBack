...
def Add_NDVI(item: Item):
    ...
    cursor.execute('SELECT coordinates FROM field WHERE id =%s',(item.id_field,)) 
    for row in cursor:
        print(row)
    ...
    cursor.execute("INSERT INTO ndvi (ndvimap, id_field, startdate, enddate, type) VALUES(%s, %s, %s, %s, %s) ON CONFLICT (ndvimap) DO NOTHING",
     (path,item.id_field,item.startdate, item.enddate, item.ndvi))
    conn.commit()
